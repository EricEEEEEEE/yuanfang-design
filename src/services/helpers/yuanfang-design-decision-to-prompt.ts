...
export function designDecisionPromptLines(decision: YuanfangDesignDecision): string[] {
  return [
    ...
    `- overlayReserveMode: ${decision.overlayReserveMode}`,
    `- titleSafeGeometry: shape ${decision.titleSafeGeometry.shape}; maxCanvasAreaRatio ${decision.titleSafeGeometry.maxCanvasAreaRatio}; preferredAreaRatioRange ${decision.titleSafeGeometry.preferredAreaRatioRange.join("-")}; keep it as an implicit overlay reserve.`,
