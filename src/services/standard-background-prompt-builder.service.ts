...
function buildPrompt(context: StandardImagePromptContext, templates: ReturnType<typeof loadTemplates>, keys: TemplateKeys, visualRules: StandardBackgroundVisualRuleContext): string {
  ...
  return [
    ...
    "Reserve a compact designed title-safe zone according to the selected geometry: usually 12%-28% canvas area, stage zones max 35%, never titleSafeAreaOver40Percent; keep subtle structure, material, boundary, and depth; do not place detailed objects, faces, icons, strong contrast, or text-like patterns inside it; it must be anchored to the subject, not a full-height side panel, full-width panel, giant plaque, central paper sheet, or empty curtain.",
    ...
    "Reserved title-safe areas must stay small enough for the background to remain visible around them; bind the title-safe shape to a ribbon, side segment, plaque edge, stage beam, book layer, or brand color boundary.",
    "Avoid oversizedTitleSafeBoard, fullHeightBlankPanel, giantEmptyPlaque, emptySpotlightCurtain, centralPaperSheetDominance, disconnectedTitleIsland, overblankTitleZone, and centerBlankBoard patterns.",
