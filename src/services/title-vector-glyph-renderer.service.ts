...
import type { TitleFontResolveResult, VectorGlyphRenderInput, VectorGlyphRenderResult, VectorGlyphRun, VectorGlyphSafetyCheck, VectorGlyphWarning, VectorTitleLayer, VectorTitleRole } from "@/models/title-vector-glyph-renderer";

type RunStyle = { fill: string; strokeWidth: number; letterSpacing: number; lineHeightFactor: number; widthFactor: number };
type FontFaceAsset = { fontKey: string; family: string; css: string; filePath: string; loaded: boolean; warning?: string };
type FitResult = { fontSize: number; measured: TitleBox; fits: boolean };

const fontFaceCache = new Map<string, FontFaceAsset>();
...
export function renderTitleVectorGlyph(input: VectorGlyphRenderInput): VectorGlyphRenderResult {
  ...
  const mainRuns = orderedUnits.map((unit, index) => createRun(input, unit.text, unit.visualRole, unit.unitBox, unit.visualWeight, unit.allowEmphasis, unit.alignment, `${bp.candidateId}-unit-${index + 1}`, warnings, fontAssets, unit.direction));
  const subtitleRun = createSubtitleRun(input, warnings, fontAssets);
  const glyphRuns = subtitleRun ? [...mainRuns, subtitleRun] : mainRuns;
  const layers = createLayers(glyphRuns, input.renderMode === "debug");
  const measuredBoxes = {
    lockupBox: box(bp.lockupBox),
    unitBoxes: mainRuns.map((run) => ({ text: run.text, planned: run.plannedBox, measured: run.measuredBox })),
    ...(subtitleRun ? { subtitleBox: { text: subtitleRun.text, planned: subtitleRun.plannedBox, measured: subtitleRun.measuredBox } } : {}),
  };
  const checks = createSafetyChecks(input, titleJoin, mainRuns, glyphRuns);
  const safety = { passed: checks.every((check) => check.passed || check.severity !== "error"), checks };
  const svg = renderRealSvg(input, glyphRuns, fontAssets);
  return { source: "vector-glyph-renderer-v1", candidateId: bp.candidateId, sourceCandidateId: sourceCandidateId(input), svg, sharpLayer: input.outputFormat === "sharpLayer" ? { input: Buffer.from(svg), top: 0, left: 0 } : undefined, layers, measuredBoxes, glyphRuns, safety, warnings, reason: "real SVG font render v1; measurement is deterministic estimate only, no Sharp raster measurement." };
}

function createRun(input: VectorGlyphRenderInput, text: string, role: VectorTitleRole, plannedBox: TitleUnitBox, visualWeight: number, allowEmphasis: boolean, alignment: string, runId: string, warnings: VectorGlyphWarning[], fontAssets: FontFaceAsset[], direction?: string): VectorGlyphRun {
...
function createSubtitleRun(input: VectorGlyphRenderInput, warnings: VectorGlyphWarning[], fontAssets: FontFaceAsset[]): VectorGlyphRun | null {
  ...
  return createRun(input, input.blueprint.subtitleLockup.text, "subtitle", subtitleBox, input.blueprint.subtitleLockup.visualWeight, false, "center", `${input.blueprint.candidateId}-subtitle`, warnings, fontAssets);
}

function createSafetyChecks(input: VectorGlyphRenderInput, titleJoin: string, mainRuns: readonly VectorGlyphRun[], glyphRuns: readonly VectorGlyphRun[]): VectorGlyphSafetyCheck[] {
  const bp = input.blueprint;
  const mainRunsJoin = mainRuns.map((run) => run.text).join("");
  const zones = input.safetyContext?.forbiddenZones ?? [];
  const forbiddenOverlap = zones.some((zone) => glyphRuns.some((run) => run.measuredBox && overlaps(run.measuredBox, zone)));
  const subtitleRun = glyphRuns.find((run) => run.role === "subtitle");
  const subtitleOverlap = subtitleRun?.measuredBox ? mainRuns.some((run) => run.measuredBox && overlaps(run.measuredBox, subtitleRun.measuredBox!)) : false;
  const measuredInside = glyphRuns.every((run) => run.measuredBox && inside(run.measuredBox, run.plannedBox));
  const fontSeverity = input.renderMode === "production" ? "error" : "warning";
  const zoneSeverity = input.renderMode === "production" ? "error" : "warning";
  return [
    ...
    check("font_embedded", glyphRuns.every((run) => run.fontEmbedded), fontSeverity, "all resolved local fonts should be embedded as @font-face."),
...
function renderRealSvg(input: VectorGlyphRenderInput, runs: readonly VectorGlyphRun[], assets: readonly FontFaceAsset[]): string {
  ...
  const debug = input.renderMode === "debug" ? renderDebugOverlay(input, runs) : "";
  return `<svg width="${input.canvas.width}" height="${input.canvas.height}" viewBox="0 0 ${input.canvas.width} ${input.canvas.height}" xmlns="http://www.w3.org/2000/svg"><defs><style><![CDATA[${css}
...
function fitTextBox(text: string, planned: TitleUnitBox, style: RunStyle, alignment: string): FitResult {
  let low = 8;
  let high = Math.max(8, planned.height * 0.88);
  let best = low;
  for (let i = 0; i < 18; i += 1) {
    const mid = (low + high) / 2;
    const measured = estimateTextBox(text, mid, style);
    if (measured.width <= planned.width && measured.height <= planned.height) { best = mid; low = mid; } else high = mid;
  }
...
function estimateTextBox(text: string, fontSize: number, style: RunStyle): TitleBox {
  ...
  const width = count * fontSize * style.widthFactor + Math.max(0, count - 1) * style.letterSpacing + style.strokeWidth * 2;
  const height = fontSize * style.lineHeightFactor + style.strokeWidth * 2;
  return { x: 0, y: 0, width: round(width), height: round(height) };
}

function loadFontFace(font: TitleFontResolveResult): FontFaceAsset {
  if (!font.filePath || !font.resolvedFontKey) return { fontKey: font.resolvedFontKey ?? "system", family: font.family, css: "", filePath: font.filePath ?? "", loaded: false, warning: "no local font filePath; using system family fallback only" };
  const absolutePath = path.resolve(process.cwd(), font.filePath);
  const cached = fontFaceCache.get(absolutePath);
  if (cached) return cached;
  if (!existsSync(absolutePath)) return cacheFont(absolutePath, { fontKey: font.resolvedFontKey, family: font.family, css: "", filePath: font.filePath, loaded: false, warning: "local font file does not exist" });
  ...
  return cacheFont(absolutePath, { fontKey: font.resolvedFontKey, family: font.family, css, filePath: font.filePath, loaded: true });
}
...
function baseWarnings(input: VectorGlyphRenderInput): VectorGlyphWarning[] {
  const warnings = [
    warning("estimated_measurement_only", "estimated measurement only; true Sharp raster measurement is not implemented in this stage.", "measuredBoxes"),
    warning("sharp_raster_measurement_not_implemented", "Sharp raster measurement not implemented.", "safety"),
    warning("glyph_path_not_implemented", "glyph path conversion is intentionally not implemented in Real SVG v1.", "glyphRuns"),
    warning("shadow_glow_bleed_warning", "shadow/glow bleed is warning-only unless future raster measurement proves overflow.", "svg"),
  ];
...
function widthFactor(fontKey: string | null): number { return fontKey?.includes("Serif") || fontKey?.includes("Wenkai") ? 1 : fontKey?.includes("Marker") || fontKey?.includes("Rounded") ? 0.98 : 0.95; }
